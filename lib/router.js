...
Router.route('join', {
    data: function() {
        console.log('join#data: Meteor.user=', Meteor.user());
        return Meteor.user();
    }
});
Router.route('signin');
...
Router.route('newCard', {
    path: '/lessons/:_id/newCard',
    onBeforeAction: function () {
    },
    data: function () {
        return {name: "New Card"};//Lessons.findOne(this.params._id);
    },
    action: function () {
        this.render();
    }
...
Router.route('lessonShow', {
  path: '/lessons/:_id',
  // subscribe to todos before the page is rendered but don't wait on the
  // subscription, we'll just render the items as they arrive
  onBeforeAction: function () {
    this.lessonsHandle = Meteor.subscribe('lessons');
    this.cardsHandle = Meteor.subscribe('cards', this.params._id);
    if (this.ready()) {
      // Handle for launch screen defined in app-body.js
      dataReadyHold.release();
    }
